...
def constructMatrixFromFile(alignmentData, fastaData):
    '''
    Reads the input file (generated by either the align_all.py or
    bootstrap_and_align.py programs) into a distance matrix.
    @param alignmentData A .txt file output by either the align_all.py or
                         bootstrap_and_align.py programs. Contains a series of sets
                         of 3 lines, which are the pairwise alignments of 2
                         sequences together with their scores. These look like:
...
def getNeighborJoiningPhylogeny(sequences, distMatrix):
    ...
    for i in range(numSeqs):
        for j in range(numSeqs):
            distPrime[i][j] = (numSeqs - 2)*distMatrix[i][j] - distFromAllOthers[i] \
                                - distFromAllOthers[j]
    ...
    if DEBUGGING:
        ...
        for row in distPrime:
            print(row)
        print( "Lowest dprime: ", pairLoc, "\n")


    newNodeName = "(" + pairWithLowestDPrime[0].getReadableIdentifier() + ", "\
    ...
    sequences.remove(pairWithLowestDPrime[0])
    sequences.remove(pairWithLowestDPrime[1])
